...
abstract class EstLin[T] {
  ...
  /** @return Valor do elemento "inicial", "primeiro" ou "cabeça" da estrutura linear, se existir. */
  def head: Option[T]
    
  /** @return Valor do elemento "final", "último" ou "cauda" da estrutura linear, se existir. */
  def tail: Option[T]

  /** @return Quantidade de elementos atualmente na estrutura linear. */
  def size: Int
