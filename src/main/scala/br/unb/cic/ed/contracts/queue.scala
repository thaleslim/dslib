...
abstract class Queue[T] extends EstLin[T] {
  ...
  /** @return Valor do elemento "inicial", "primeiro" ou "cabeça" da fila
   */
  def head: Option[T]
  
  /** @return Valor do elemento "final", "último" ou "cauda" da fila
   */
  def tail: Option[T]
  
  /** @return Quantidade de elementos atualmente na fila
   */
  def size: Int
